...
const TEMPLATE_ID = null; // paste from the template's URL in the UI (no list-templates API!)

if (!KEY) { console.error("Set RL_API_KEY first (Settings -> API -> Create API key)."); process.exit(1); }
...
async function rl(path, { method = "GET", body, attempt = 1 } = {}) {
  ...
  if (res.status === 429 && attempt <= 5) {
    ...
    const waitMs = Number.isFinite(retryAtMillis) && retryAtMillis > Date.now()
      ? retryAtMillis - Date.now()
      : Math.min(2 ** attempt * 1000, 30_000) + Math.random() * 500; // backoff + jitter fallback
    console.log(`429 rate-limited. Honoring X-Retry-After: waiting ${(waitMs / 1000).toFixed(1)}s (attempt ${attempt})`);
  ...
  if (!res.ok) {
    console.error(`HTTP ${res.status} (X-Request-Id: ${reqId})`, JSON.stringify(json?.errors ?? json, null, 2));
...
async function lab1() {
  console.log("== Users (API is read-only for users - invites are side effects) ==");
  const users = await rl("/users?pageSize=10");
  for (const u of users.data ?? []) console.log(` - ${u.firstName ?? ""} ${u.lastName ?? ""} <${u.email}> [${u.type}/${u.status}]`);
  ...
  do {
    ...
    console.log(` page ${page}: ${(res.data ?? []).map(p => `#${p.projectId} ${p.projectName}`).join(" | ") || "(empty)"}`);
    console.log(`   pagination: hasMore=${res.pagination?.hasMore} total=${res.pagination?.totalRecordCount}`);
    token = res.pagination?.hasMore ? res.pagination?.nextPageToken : null;
    page++;
  } while (token);

  console.log("\n== Filter syntax: operator suffixes ==");
  const filtered = await rl(`/projects?projectName.cn=onboarding&sortBy=createdAt&sortOrder=DESC`);
  console.log(` projects containing 'onboarding': ${filtered.data?.length ?? 0}`);
}

// ---- Lab 2: create projects (THE likely assignment) ----
async function lab2() {
  console.log("== 2a. Minimal create: the exactly-three required fields ==");
  const p1 = await rl("/projects", { method: "POST", body: {
  ...
  console.log(` created #${p1.projectId ?? p1.data?.projectId} -> 201 with full project object`);

  console.log("\n== 2b. With dates, visibility, financials, idempotency key ==");
  const ext = `lab-${Date.now()}`;
  const p2 = await rl("/projects", { method: "POST", body: {
  ...
  console.log(` lookup by externalReferenceId found ${found.data?.length ?? 0} project(s) - safe-retry pattern proven`);

  if (TEMPLATE_ID) {
    console.log("\n== 2c. From a template (sources[]; startDate REQUIRED with sources) ==");
    const p3 = await rl("/projects", { method: "POST", body: {
    ...
    console.log(` created from template ${TEMPLATE_ID}: #${p3.projectId ?? p3.data?.projectId}`);
  } else {
...
// ---- Lab 3: phases + tasks ----
async function lab3() {
  const proj = await rl("/projects", { method: "POST", body: {
    projectName: `API lab tasks ${Date.now()}`,
    customer: { companyName: "Acme Lab Inc" }, owner: { emailId: OWNER_EMAIL }, autoCreateCompany: true,
  }});
  const pid = proj.projectId ?? proj.data?.projectId;
  console.log(`project #${pid}`);

  console.log("== Phase: needs phaseName + project + BOTH dates ==");
  const phase = await rl("/phases", { method: "POST", body: {
    phaseName: "Kickoff", project: { projectId: pid }, startDate: "2026-08-03", dueDate: "2026-08-14",
  }});
  const phaseId = phase.phaseId ?? phase.data?.phaseId;

  console.log("== Task: taskName + project required; assignees via the SUB-ENDPOINT, not PUT ==");
  const task = await rl("/tasks", { method: "POST", body: {
    taskName: "Draft onboarding plan", project: { projectId: pid },
    phase: phaseId ? { phaseId } : undefined,
    type: "TASK", effortInMinutes: 120, startDate: "2026-08-03", dueDate: "2026-08-05",
  }});
  const taskId = task.taskId ?? task.data?.taskId;
  await rl(`/tasks/${taskId}/add-assignees`, { method: "POST", body: { members: [{ emailId: OWNER_EMAIL }] } });
...
async function lab4() {
  ...
  const entry = await rl("/time-entries", { method: "POST", body: {
    date: today, minutes: 120, activityName: "API lab study", billable: false, notes: "Shreeda's 'log two hours' example, raw",
  }});
  console.log(`time entry created for ${today}: 120 min (id ${entry.timeEntryId ?? entry.data?.timeEntryId})`);
}

// ---- Lab 5: deliberately show the throttle logic ----
async function lab5() {
  console.log("Firing 12 list-GETs with a client-side throttle (their budget: 60/min on list-GETs)...");
  let done = 0;
  for (let i = 0; i < 12; i++) {
  ...
  console.log(`${done}/12 succeeded with zero 429s - the point: budget proactively; the 429 handler is the safety net, not the plan.`);
}

const labs = { lab1, lab2, lab3, lab4, lab5 };
const pick = process.argv[2];
if (!labs[pick]) { console.log(`Usage: node round2-labs.mjs <${Object.keys(labs).join("|")}>`); process.exit(1); }
labs[pick]().catch(e => { console.error(e.message); process.exit(1); });
